...
// Scaffold qstruct and qunion additions.
bool Type::isQstructureType() const {
  if (const RecordType *RT = getAs<RecordType>())
    return RT->getDecl()->isQstruct();
  return false;
}

bool Type::isQunionType() const {
  if (const RecordType *RT = getAs<RecordType>())
    return RT->getDecl()->isQunion();
  return false;
}
...
// Scaffold Additions for qstuct and qunion.
const RecordType *Type::getAsQstructureType() const {
  // If this is directly a qstructure type, return it.
  if (const RecordType *RT = dyn_cast<RecordType>(this)) {
    if (RT->getDecl()->isQstruct())
      return RT;
  }
...
const RecordType *Type::getAsQunionType() const {
  // If this is directly a union type, return it.
  if (const RecordType *RT = dyn_cast<RecordType>(this)) {
    if (RT->getDecl()->isQunion())
      return RT;
  }
